...
template <class D>
void Print(const D d, const char* caption, const decltype(Zero(d)) v,
...
void AssertVecEqual(D d, V expected, V actual, const char* filename, int line) {
  auto expected_lanes = AllocateAligned<TFromD<D>>(Lanes(d));
